...
public class ReportService {
    ...
    public boolean reportTopStudents(int n, String fileName) {
        ...
        if (n > 100) { n = 100; limited = true; }
        if (n < 0) throw new IllegalArgumentException("invalid n");

        List<Registry.StudentRow> rows = registry.topStudents(n);
        writeByExt(fileName,
                () -> CsWriter.writeTopStudents(Path.of(fileName), rows),
                () -> JsonWriter.writeTopStudents(Path.of(fileName), rows),
                () -> XmlWriter.writeTopStudents(Path.of(fileName), rows),
                );
        return limited;
    }

    public boolean reportTopCourses(int n, String fileName) {
        Validation.requireLoaded(required.isLoaded());
    ...
    public void reportTranscript(String studentId, String fileName) {
        ...
        if (t == null) {
            System.out.println("error: no student found for the given id");
            return;
        }

        writeByExt(fileName,
                () -> CsWriter.writeTopStudents(Path.of(fileName), t),
                () -> JsonWriter.writeTopStudents(Path.of(fileName), t),
                () -> XmlWriter.writeTopStudents(Path.of(fileName), t),
                );
